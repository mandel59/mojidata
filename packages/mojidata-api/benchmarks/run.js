...
function getSelectedScenarios(names) {
    if (names.length === 0)
        return scenarios;
...
function createUrl(pathname, query, baseUrl) {
...
async function timedRequest(execute, scenario, baseUrl) {
    ...
    if (!response.ok) {
        throw new Error(`Scenario ${scenario.name} failed with ${response.status} ${response.statusText}: ${text.slice(0, 200)}`);
    }
    try {
        JSON.parse(text);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : `Unknown parse error: ${String(error)}`;
...
function printTable(results, options) {
    ...
    console.log(`mojidata-api benchmark: ${mode}\n` +
    ...
    const rows = results.map((result) => {
        ...
        const coldSummary = result.coldStartMs && result.coldStartMs.length > 0
    ...
    const formatRow = (row) => row.map((value, columnIndex) => value.padEnd(widths[columnIndex])).join('  ');
...
function printJson(results, options) {
    const payload = {
        ...
        results: results.map((result) => ({
            ...
            coldSummary: result.coldStartMs && result.coldStartMs.length > 0
                ? summarize(result.coldStartMs)
                : undefined,
        })),
    };
...
async function runScenario(scenario, options) {
    ...
    if (options.coldIterations > 0) {
        ...
        for (let index = 0; index < options.coldIterations; index += 1) {
            ...
            result.coldStartMs.push(await timedRequest((url) => app.fetch(new Request(url)), scenario, baseUrl));
        }
    }
    const app = (0, node_1.createNodeApp)();
    for (let index = 0; index < options.warmupIterations; index += 1) {
    ...
    for (let index = 0; index < options.iterations; index += 1) {
        result.samplesMs.push(await timedRequest((url) => app.fetch(new Request(url)), scenario, baseUrl));
    }
    return result;
}
async function main() {
