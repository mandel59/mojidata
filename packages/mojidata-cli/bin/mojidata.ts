...
function printMojidata(s: string) {
    ...
    for (const c of s) {
        ...
        SELECT json_object(
            ...
            'unihan', (SELECT json_group_object(property, value) FROM unihan_draft as unihan WHERE unihan.UCS = @ucs),
            'unihan_variant', (SELECT json_group_array(CASE WHEN additional_data IS NOT NULL THEN json_array(property, printf('U+%04X', unicode(value)), value, additional_data) ELSE json_array(property, printf('U+%04X', unicode(value)), value) END) FROM unihan_draft_variant as unihan_variant WHERE unihan_variant.UCS = @ucs),
